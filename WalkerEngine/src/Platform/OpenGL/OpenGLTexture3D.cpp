...
namespace Walker {
		...
		: m_Path(path)
	{
		...
		{
			//W_PROFILE_SCOPE("stbi_load - OpenGLTexture3D::OpenGLTexture3D(const std::string&)");
	...
	OpenGLTexture3D::OpenGLTexture3D(const TextureSpecification& spec)
	{
		...
		glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_S, Utils::WalkerTextureWrapTypeToGL(spec.WrapS));
	...
	void OpenGLTexture3D::SetData(void* data, uint32_t size)
	{
		...
		//glBindTexture(GL_TEXTURE_3D, m_RendererID);
		//glTexImage3D()
		//glTexImage3D(GL_TEXTURE_3D, 0, m_InternalFormat, m_Width, m_Height, m_Depth, 0, m_DataFormat, GL_UNSIGNED_BYTE, data);
	...
	void OpenGLTexture3D::BindImage(uint32_t slot, TextureAccess access) const
	{
