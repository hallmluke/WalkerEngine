...
namespace Walker {
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		if (s_GLFWWindowCount == 0)
		{
			...
			if (!success) {
				W_CORE_ERROR("glfw loader failed");
			}
	...
	void WindowsWindow::SetIcon(std::string icon) const
	{
		int width, height, nrComponents;
		unsigned char* data = stbi_load(icon.c_str(), &width, &height, &nrComponents, 0);
		if (data) {
			GLFWimage icons[1];
			icons[0].pixels = data;
			icons[0].width = width;
		...
		{
			std::cout << "Failed to load icon: " << icon << std::endl;
			stbi_image_free(data);
		}
	}
